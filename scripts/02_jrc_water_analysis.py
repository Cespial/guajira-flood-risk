...
========================
Analysis of the JRC Global Surface Water (GSW) v1.4 dataset for La Guajira.
...
def run_jrc_analysis(export: bool = True) -> list:
    ...
    if export:
        for name, image in jrc_layers.items():
            task = export_to_drive(
                image=image.toFloat(),
                description=f"guajira_jrc_{name}",
                region=region,
    ...
    if export:
        task = export_to_drive(
            ...
            description="guajira_jrc_flood_frequency",
            region=region,
            scale=cfg.EXPORT_SCALE,
        )
        tasks.append(task)
    ...
    if export:
        ...
        task = export_to_drive(
            ...
            description="guajira_jrc_seasonal_dynamics",
            region=region,
            scale=cfg.EXPORT_SCALE,
        )
        tasks.append(task)
    ...
    if export:
        task = export_to_drive(
            ...
            description="guajira_jrc_water_trends",
            region=region,
            scale=cfg.EXPORT_SCALE,
        )
        tasks.append(task)
    ...
    if export:
        task = export_table_to_drive(
            ...
            description="guajira_jrc_municipal_occurrence_stats",
            file_format="CSV",
        )
        tasks.append(task)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="JRC Global Surface Water analysis for La Guajira",
    )
    parser.add_argument(
        "--no-export",
        action="store_true",
