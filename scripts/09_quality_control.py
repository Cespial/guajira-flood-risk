...
=====================
Quality control and validation for the La Guajira Flood Risk Assessment.
...
def verify_municipal_stats() -> List[QCResult]:
    ...
    if exposure_path.exists():
        ...
        if "Total Population" in exp_df.columns and "Exposed Population" in exp_df.columns:
            data_rows = exp_df[exp_df["Subregion"] != "TOTAL (La Guajira)"]
...
def generate_qc_report() -> pathlib.Path:
    ...
    lines.append(f"## La Guajira Flood Risk Assessment")
    lines.append("")
