...
as PNG arrays, then overlays on proper La Guajira boundaries (WGS84).
...
from utils import (
    load_guajira_boundary, load_municipalities,
...
def get_guajira_bbox(region):
    """Get the bounding box of La Guajira as [west, south, east, north]."""
...
def fig03_jrc_water_occurrence():
    """Download real JRC water occurrence and overlay on La Guajira boundary (WGS84)."""
    print("Generating Figure 3: JRC water occurrence (REAL DATA)...")
    set_publication_style()

    region = get_guajira_region()
    ...
    guajira_gdf.boundary.plot(ax=ax, color="black", linewidth=1.2)

    # Set axis limits to La Guajira extent with small padding
    pad = 0.05
