...
07_visualization.py
===================
Publication-quality figure generation for the La Guajira Flood Risk Assessment.
...
def fig01_study_area() -> None:
    ...
    # Figure with two panels: (a) Colombia context, (b) La Guajira detail
    ...
    try:
        ...
        col_depts.plot(ax=ax_context, color="#f0f0f0", edgecolor="#999999",
                       linewidth=0.3)
        # Highlight La Guajira
        guajira_proj.plot(ax=ax_context, color="#fc8d59", edgecolor="black",
                           linewidth=0.8)
    except Exception:
        guajira_proj.plot(ax=ax_context, color="#fc8d59", edgecolor="black",
                           linewidth=0.8)
    ...
    # La Guajira border (thick)
    guajira_proj.boundary.plot(ax=ax_main, color="black", linewidth=1.2)
    ...
    ax_main.set_title("(b) La Guajira subregions", fontsize=9, fontweight="bold")
...
def fig03_jrc_water_occurrence() -> None:
    """
    JRC Global Surface Water occurrence map for La Guajira, showing
    permanent and seasonal water bodies.
    """
    logger.info("Generating Figure 3: JRC water occurrence map...")
    set_publication_style()

    guajira = load_guajira_boundary("gadm").to_crs(CRS_COLOMBIA)

    # Load or synthesize JRC occurrence raster
    jrc_data = _load_or_synthesize_raster(
        "phase1_water_maps", "jrc_occurrence_guajira.tif",
        shape=(600, 500), vmin=0, vmax=100,
    )

    fig, ax = plt.subplots(figsize=figsize_single(1.1))

    # Plot La Guajira background
    guajira.plot(ax=ax, facecolor="#f5f5f5", edgecolor="black", linewidth=0.8)

    # Overlay JRC occurrence as imshow (placeholder extent)
    bounds = guajira.total_bounds  # [minx, miny, maxx, maxy]
    im = ax.imshow(
        jrc_data,
...
def fig04_flood_frequency() -> None:
    ...
    guajira.plot(ax=ax, facecolor="#f0f0f0", edgecolor="black", linewidth=0.8)

    bounds = guajira.total_bounds
    im = ax.imshow(
        flood_freq,
        extent=[bounds[0], bounds[2], bounds[1], bounds[3]],
        origin="upper",
...
def fig05_hand_map() -> None:
    ...
    hand_data = _load_or_synthesize_raster(
        "phase2_flood_frequency", "hand_guajira.tif",
        shape=(600, 500), vmin=0, vmax=100,
    )
...
def fig08_susceptibility_map() -> None:
    """
    Final ensemble flood susceptibility map for La Guajira, displayed
...
def fig10_population_exposure() -> None:
    ...
    try:
        ...
        guajira.boundary.plot(ax=ax_map, color="black", linewidth=0.8)
    except Exception:
