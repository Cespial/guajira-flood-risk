...
======================
Climate analysis for flood context in La Guajira, Colombia.
...
# ---------------------------------------------------------------------------
# GEE helper: get La Guajira geometry
# ---------------------------------------------------------------------------

def get_guajira_geometry() -> ee.Geometry:
    """Return the La Guajira department geometry from FAO GAUL."""
    admin = ee.FeatureCollection(ADMIN_DATASET)
    guajira = admin.filter(ee.Filter.eq("ADM1_NAME", DEPARTMENT_NAME))
    return guajira.geometry()
...
def compute_temperature_trends() -> pd.DataFrame:
    ...
    MODIS MOD11A2 (8-day composites) over La Guajira.
...
def compute_spi(
    ...
    """
    Compute the Standardized Precipitation Index (SPI) for La Guajira
...
def extreme_precipitation_events(
    ...
    threshold computed from the full CHIRPS record over La Guajira.
...
def precipitation_flood_correlation(
    ...
    SAR-detected flood extent over La Guajira.
...
def _plot_precipitation_trends(precip_df: pd.DataFrame) -> None:
    ...
    ax.set_title("(a) Annual precipitation trend, La Guajira (CHIRPS)")
...
def _plot_spi_timeseries(spi_df: pd.DataFrame) -> None:
    ...
    ax.set_ylabel("SPI-3")
    ax.set_xlabel("Date")
    ax.set_title("Standardized Precipitation Index (SPI-3), La Guajira")
