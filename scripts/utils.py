...
Author : La Guajira Flood Risk Research Project
Date   : 2026-02-26
"""
...
def get_study_area() -> ee.FeatureCollection:
    """
    Return the La Guajira department boundary from FAO GAUL Level 1.

    Uses the ``FAO/GAUL/2015/level1`` dataset and filters for
    La Guajira in Colombia.  The result is a single-feature
    ``ee.FeatureCollection`` suitable for clipping and region operations.

    Returns
    -------
    ee.FeatureCollection
        La Guajira department polygon.
    """
    guajira = (
...
def get_study_area_geometry() -> ee.Geometry:
    """
    Return the dissolved geometry of La Guajira as a single ``ee.Geometry``.
...
def get_municipalities() -> ee.FeatureCollection:
    """
    Return all municipalities within La Guajira from FAO GAUL Level 2.

    Returns
    -------
    ee.FeatureCollection
        ~15 municipality polygons with properties including ``ADM2_NAME``.
    """
    municipalities = (
...
def get_subregion_municipalities(subregion_name: str) -> ee.FeatureCollection:
    """
    Filter municipalities belonging to a specific La Guajira subregion.

    Parameters
    ----------
    subregion_name : str
        One of the 3 official La Guajira subregions defined in
        ``gee_config.SUBREGIONS`` (e.g. ``'Alta Guajira'``).
...
def get_dem() -> ee.Image:
    """
    Load the SRTM 30 m DEM clipped to La Guajira.
...
def _self_test() -> None:
    ...
    # Study area
    guajira = get_study_area()
    n_features = safe_getinfo(guajira.size(), label="guajira.size()")
    _log.info("La Guajira feature count: %s", n_features)
    ...
    # Subregion filter
    ag = get_subregion_municipalities("Alta Guajira")
    n_ag = safe_getinfo(ag.size(), label="alta_guajira.size()")
    _log.info("Alta Guajira municipalities: %s", n_ag)
