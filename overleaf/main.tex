...
Unlike Antioquia's bimodal seasonality, La Guajira's flood dynamics are expected to show a pronounced unimodal pattern with virtually zero flood extent during December--March (dry season) and a sharp September--November peak. The ENSO amplification is expected to be even stronger than the 34\% observed in Antioquia, given La Guajira's extreme sensitivity: CHIRPS-based analyses show that El Ni\~no reduces rainfall by 30--50\% across the department \citep{GallegoRevilla2022}, while La Ni\~na can increase it by 20--40\%.

\begin{table}[htbp]
\caption{Flood extent statistics by ENSO phase. To be populated after pipeline execution.}
\label{tab:enso}
\centering
\footnotesize
