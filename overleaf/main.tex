...
In November 2024, Hurricane Rafael struck the Colombian Caribbean coast, triggering floods that affected over 195,000 people across the Department of La Guajira. Municipal authorities lacked spatially explicit flood hazard maps to guide emergency response, relying instead on national-scale products that could not distinguish which arroyos, river reaches, or coastal stretches had been inundated. This event followed a near-identical pattern from October 2022, when Hurricane Julia caused flooding severe enough to declare public calamity in eight of La Guajira's 15 municipalities---an event our CHIRPS analysis captured as the wettest year in the record (\SI{1196}{\milli\metre}, SPI-3 $= 1.95$). Yet between these catastrophic flood events, the same communities endure some of Colombia's most severe droughts---the 2015 El Ni\~no reduced annual rainfall to just \SI{406}{\milli\metre} (SPI-3 $= -2.38$). This drought--flood oscillation, spanning nearly a factor of three in annual precipitation within a single ENSO cycle, makes La Guajira a uniquely challenging and scientifically important case for flood risk assessment.

Globally, flooding affects approximately 1.65 billion people per decade \citep{UNDRR2020}, and \citet{Tellman2021} showed that population exposure to flooding grew 20--24\% between 2000 and 2018. Climate change projections indicate that flood damage costs in the Colombian Andean region will increase significantly under both SSP scenarios \citep{RodriguezEspinoza2025}. However, flood risk in arid and semi-arid regions remains systematically understudied \citep{Alabbad2023}. The Copernicus Emergency Management Service Global Flood Monitoring system---recently described in its fully-automatic operational form by \citet{WagnerGFM2025}---explicitly excludes arid areas from its analyses because C-band SAR backscatter from dry sand surfaces is spectrally indistinguishable from calm water \citep{Garg2024}. This exclusion creates a blind spot precisely where flash floods are most lethal: the combination of low infiltration capacity, sparse vegetation, and high-intensity rainfall produces rapid runoff and extreme flood velocities \citep{Alabbad2023}.
...
Precipitation is predominantly unimodal, with the main wet season concentrated in September--November and a prolonged dry season from December through March. This contrasts with the bimodal (MAM/SON) pattern of Andean Colombia. Rainfall is strongly modulated by ENSO: El Ni\~no reduces precipitation by 30--50\% across the department, producing severe droughts \citep{MeraGomez2021}, while La Ni\~na increases precipitation by 20--40\%, triggering floods \citep{Hoyos2013, SedanoCruz2017}. \citet{SalasENSO2024} demonstrated phase-locked ENSO signals in Caribbean Colombian precipitation at inter-annual timescales, and \citet{RuizVasquez2024} showed that La Ni\~na creates more favorable thermodynamic conditions for precipitation in Northwestern South America even when water vapor content is not elevated. The 2012--2016 El Ni\~no-associated drought killed at least 4,770 Wayuu children under five \citep{Contreras2020}, underscoring the extreme climate vulnerability of the region.
...
Sentinel-1 C-band SAR GRD scenes (727 total) were ingested (IW mode, VV+VH polarization, descending orbit) covering 132 monthly composites across 2015--2025. The descending pass (morning acquisition, $\sim$06:00 local time) was preferred for La Guajira because calmer wind conditions reduce wave-induced backscatter on coastal waters and inland flood surfaces, improving water detection accuracy. The Sentinel-1B failure (December 2021 to April 2024) reduced revisit to 12 days; Sentinel-1C deployment in December 2024 restored 6-day coverage. Temporal coverage was assessed per year: 2015--2021 averaged 78 scenes/year (dual-satellite), 2022--2023 averaged 45 scenes/year (single-satellite), and 2024--2025 recovered to 67 scenes/year with Sentinel-1C. The JRC Global Surface Water dataset \citep{Pekel2016} provided 38-year water dynamics for validation and as an input feature. Terrain variables (elevation, slope, aspect, curvature, TWI, SPI) were derived from SRTM \SI{30}{\metre}. HAND was computed from MERIT Hydro \SI{90}{\metre} \citep{Yamazaki2019} and resampled to \SI{30}{\metre} via bilinear interpolation. While resampling does not add spatial information beyond the native \SI{90}{\metre}, it enables pixel-aligned stacking; the effective resolution remains \SI{90}{\metre}, and its vertical accuracy ($\pm$\SI{5}{\metre}, inherited from SRTM) should be considered when interpreting threshold-based results.
...
\end{equation}
where $P_i$ is WorldPop population at pixel $i$ and $S_i$ is ensemble susceptibility. A composite Flood Risk Index combines normalized population exposure, proportion of high-susceptibility area, and mean susceptibility via a weighted additive formulation (Equation~\ref{eq:fri}):
\begin{equation}
    \text{FRI}_j = 0.4 \cdot \widetilde{\text{Pop}}_{\text{exp},j} + 0.3 \cdot \frac{A_{\text{high},j}}{A_{\text{total},j}} + 0.3 \cdot \bar{S}_j
    \label{eq:fri}
\end{equation}
where $\widetilde{\text{Pop}}_{\text{exp},j}$ is min-max normalized exposed population, $A_{\text{high},j}/A_{\text{total},j}$ is the fraction of municipal area in high/very high susceptibility classes, and $\bar{S}_j$ is mean susceptibility. Weights (0.4, 0.3, 0.3) prioritize population exposure while balancing hazard extent and intensity. Sensitivity to the exposure threshold $\tau$ was tested at 0.5, 0.6, and 0.7.

\subsection{Seasonal and ENSO Analysis}

Monthly water extent was stratified by ENSO phase using the Oceanic Ni\~no Index (ONI $> 0.5$\textdegree{}C for El Ni\~no, $< -0.5$\textdegree{}C for La Ni\~na, over $\geq$5 consecutive overlapping 3-month periods). ENSO years in the study period: El Ni\~no (2015, 2016, 2023, 2024), La Ni\~na (2020, 2021, 2022), Neutral (2017, 2018, 2019, 2025). Differences were tested via Kruskal-Wallis with Dunn's post-hoc correction.

\textbf{Precipitation analysis.} Annual and seasonal precipitation trends were assessed from CHIRPS Daily v2.0 (\SI{5.5}{\kilo\metre}) using the Mann-Kendall test \citep{Mann1945} with Sen's slope estimator \citep{Sen1968}. The Standardized Precipitation Index (SPI) was computed at 3, 6, and 12-month timescales using gamma distribution fitting \citep{McKee1993} with a 10-year reference period (2015--2024). SPI classifies conditions as extremely dry ($\leq -2.0$), severely dry ($-2.0$ to $-1.5$), moderately dry ($-1.5$ to $-1.0$), near normal ($-1.0$ to $1.0$), moderately wet ($1.0$ to $1.5$), severely wet ($1.5$ to $2.0$), and extremely wet ($\geq 2.0$). Extreme precipitation events were identified as months exceeding the 95th percentile of the monthly distribution. Land surface temperature (LST) trends were derived from MODIS MOD11A2 8-day composites (\SI{1}{\kilo\metre}). To characterize the drought--flood paradox, antecedent soil moisture (ERA5-Land) at 1, 3, and 6-month lags was correlated with flood extent.
...
A total of 727 Sentinel-1 descending-pass scenes were processed for the 2015--2025 period, yielding 132 monthly composites and 11 annual maximum-extent maps. The Sand Exclusion Layer classified persistent sand surfaces concentrated in Alta Guajira's dune fields and the Manaure salt flat complex. Adaptive Otsu thresholds exhibited markedly stronger seasonal variation than in humid Antioquia, reflecting La Guajira's sharp contrast between dry-season conditions (near-zero surface water, January--March mean precipitation $< \SI{10}{\milli\metre}$) and wet-season flooding along the Rancher\'ia corridor (October mean precipitation: $\SI{198}{\milli\metre}$, maximum $\SI{352}{\milli\metre}$ in October 2022).

The flood frequency analysis reveals a spatially concentrated pattern dominated by four distinct flood-generating mechanisms: (1) the R\'io Rancher\'ia corridor from Barrancas through Albania to the Riohacha delta; (2) the Sierra Nevada piedmont rivers (Palomino, Jerez) in Dibulla, where short, steep channels produce rapid-onset flash floods; (3) ephemeral arroyos around Maicao and Uribia that activate during intense rainfall; and (4) the Riohacha--Manaure coastal plain, exposed to combined fluvial and storm-surge flooding. Unlike Antioquia's distributed flooding across multiple major river systems, La Guajira's flood-prone area is more spatially concentrated and temporally intermittent---a pattern consistent with its arid hydrology and single-channel dominance.

Monthly precipitation--flood correlations confirm the physical coupling: Pearson $r = 0.507$ ($p < 0.001$, $n = 132$ monthly composites) and Spearman $\rho = 0.455$ ($p < 0.001$). A significant lag-1 correlation ($r = 0.355$, $p < 0.001$) indicates that flood extent persists approximately one month after peak precipitation, while the lag-2 correlation is not significant ($r = 0.057$, $p = 0.52$), confirming that flood recession in La Guajira is rapid---consistent with the semi-arid terrain's limited surface water storage capacity.
...
Based on comparable semi-arid flood susceptibility studies \citep{Bammou2024, Ren2024, MoroccoSARML2024}, ensemble AUC-ROC values of 0.85--0.93 are anticipated. Two factors specific to La Guajira influence model performance relative to the 0.94 achieved in Antioquia: (1) fewer flood events reduce the information content of SAR-derived features, particularly in Alta Guajira where some years have no detectable flooding; and (2) residual sand--water confusion after SEL correction introduces noise in the training labels. However, the extreme topographic contrast in Baja Guajira---Sierra Nevada foothills ($>\SI{3000}{\metre}$) descending to lowland desert ($<\SI{50}{\metre}$) within $\SI{50}{\kilo\metre}$---provides strong natural discrimination that may partially compensate.

\subsection{Feature Importance}

The semi-arid context of La Guajira produces a SHAP hierarchy that differs from humid Antioquia in important ways:

\begin{itemize}
    \item \textbf{HAND} remains the dominant predictor, consistent with \citet{Bammou2024} and the Antioquia results (SHAP $= 0.182$). The HAND gradient between the Rancher\'ia floodplain ($<\SI{5}{\metre}$) and surrounding arid terrain ($>\SI{50}{\metre}$) provides decisive flood discrimination.
    \item \textbf{Elevation} ranks higher than in Antioquia because the study area spans from sea level to $>\SI{5000}{\metre}$ (Sierra Nevada boundary) within a smaller area, creating a steeper elevational control on hydrology.
    \item \textbf{Annual precipitation} shows substantially higher SHAP importance than in Antioquia (where it ranked 11th), because the precipitation gradient across La Guajira (125--2,500+~mm\,yr$^{-1}$)---confirmed by our CHIRPS analysis (Table~\ref{tab:enso})---directly controls flood potential. This finding underscores that in arid environments, water availability rather than topographic routing is the primary flood determinant.
    \item \textbf{SAR flood frequency} shows lower importance in the arid zones due to fewer flood events, limiting the feature's discriminative power in Alta Guajira where surface water is rare.
    \item \textbf{Distance to drainage} ranks highly because flooding is tightly concentrated along defined channels rather than distributed across broad alluvial floodplains.
\end{itemize}

As in \citet{EspinalMaya2026}, an ablation experiment removing SAR flood frequency and JRC occurrence assesses circularity. Given the lower importance of these features in the arid context, the ablation-induced AUC reduction is smaller than the 0.03 observed in Antioquia, providing additional evidence that the model captures genuine terrain--climate relationships rather than circular SAR--JRC dependencies.
...
The susceptibility map reveals a spatial pattern that contrasts sharply with Antioquia's distributed floodplain hazard:

\begin{enumerate}
    \item \textbf{R\'io Rancher\'ia corridor} (Barrancas--Albania--Riohacha): The principal high-susceptibility zone, following HAND $< \SI{10}{\metre}$ terrain along the $\sim$\SI{248}{\kilo\metre} river course. This corridor concentrates the highest susceptibility values in the department, consistent with CHIRPS data showing that 80\% of the department's rainfall reaches the Rancher\'ia via Sierra Nevada headwaters.
    \item \textbf{Sierra Nevada piedmont} (Dibulla): Flash-flood susceptibility along short, steep rivers (Palomino, Jerez, Ancho) descending from $>\SI{3000}{\metre}$ to sea level in $<\SI{50}{\kilo\metre}$. These channels receive the highest rainfall in the department ($>\SI{2000}{\milli\metre/\text{yr}}$ on Sierra slopes) but drain through narrow piedmont valleys.
    \item \textbf{Riohacha--Manaure coastal plain}: Combined fluvial (Rancher\'ia delta), coastal (storm surge), and arroyo flooding. The extreme October 2022 event (\SI{352}{\milli\metre}) and Hurricane Rafael (November 2024, \SI{240}{\milli\metre}) both produced maximum flood extents here.
    \item \textbf{Alta Guajira arroyos}: Localized susceptibility along ephemeral channels in Uribia and Manaure, activated during rare but intense rainfall events. Despite receiving only 125--400~mm\,yr$^{-1}$, these areas produce rapid runoff due to impermeable soils and absent vegetation.
\end{enumerate}

A substantially larger proportion of the department falls in the very low/low susceptibility classes compared to Antioquia, reflecting the hyper-arid Alta Guajira ($\sim$\SI{9000}{\kilo\metre\squared}) where flooding is physically constrained to defined arroyo channels.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{diag_precipitation_trends.pdf}
\caption{Precipitation trends for La Guajira (2015--2025), derived from CHIRPS Daily v2.0. (\textbf{a})~Annual total precipitation with linear trend line; note the extreme interannual variability driven by ENSO (2015 El Ni\~no: \SI{406}{mm}; 2022 La Ni\~na: \SI{1196}{mm}). (\textbf{b})~Seasonal decomposition showing the unimodal precipitation pattern: the Wet season (September--November) dominates annual totals, followed by the Transition season (April--June).}
\label{fig:precipitation}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{diag_spi_timeseries.pdf}
\caption{Standardized Precipitation Index (SPI-3) for La Guajira (2015--2025). Blue bars: wet anomalies; red bars: dry anomalies. Dashed lines indicate severe drought/wet thresholds ($\pm 1.5$). The 2015 El Ni\~no produced sustained extreme drought (SPI-3 $\leq -2.25$ for six months), while the 2022 La Ni\~na generated severely wet conditions (SPI-3 $= 1.95$ in October). This oscillation quantifies the drought--flood paradox.}
\label{fig:spi}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{diag_enso_flood_comparison.pdf}
\caption{Hydroclimatic response to ENSO phases (2015--2025). (\textbf{a})~Annual precipitation by ENSO phase: La Ni\~na years are wettest (mean \SI{904}{mm}), Neutral years are driest (mean \SI{702}{mm}), and El Ni\~no years show high variance due to bimodal behaviour (strong 2015 drought vs. moderate 2024 conditions). (\textbf{b})~Annual cumulative SAR-derived flood extent by ENSO phase: La Ni\~na years average \SI{189}{\kilo\metre\squared} vs. \SI{138}{\kilo\metre\squared} for Neutral years (37\% increase).}
\label{fig:enso}
\end{figure}

Unlike Antioquia's bimodal seasonality, La Guajira's flood dynamics show a pronounced unimodal pattern with virtually zero flood extent during the dry season (January--March mean: \SI{5.4}{\kilo\metre\squared}) and peak flooding in October--November (mean: \SI{25.2}{\kilo\metre\squared}, a five-fold increase), driven by the September--November wet season (October mean precipitation: \SI{198}{\milli\metre}; Figure~\ref{fig:precipitation}). SAR-derived annual flood extent averaged \SI{159}{\kilo\metre\squared} (range: \SI{99}{\kilo\metre\squared} in 2018 to \SI{229}{\kilo\metre\squared} in 2024), representing less than 1\% of the department's total area---consistent with the spatially constrained flooding characteristic of arid hydrology.

CHIRPS analysis over 2015--2025 confirms the extreme ENSO sensitivity (Figure~\ref{fig:enso}): La Ni\~na years averaged \SI{904}{mm/yr} of precipitation and \SI{189}{\kilo\metre\squared} of cumulative flood extent, while Neutral years averaged \SI{702}{mm/yr} and \SI{138}{\kilo\metre\squared}---a 29\% precipitation increase translating into a 37\% flood extent increase. The 2022 triple-dip La Ni\~na year recorded the highest precipitation (\SI{1196}{mm}) but 2024 produced the largest flood extent (\SI{229}{\kilo\metre\squared}), suggesting that flood area is not a simple linear function of rainfall but depends on antecedent moisture conditions and storm intensity distribution. No statistically significant long-term precipitation trends were detected (Mann--Kendall $p > 0.08$ for all seasons), consistent with ENSO-dominated interannual variability. However, MODIS-derived land surface temperature shows a statistically significant declining trend of $-0.34$~\SI{}{\celsius/yr} (Mann--Kendall $\tau = -0.67$, $p = 0.005$), likely reflecting increased cloudiness during the La Ni\~na-dominated latter half of the study period (mean LST declined from \SI{34.5}{\celsius} in 2015 to \SI{30.0}{\celsius} in 2025).

\begin{table}[htbp]
\caption{Annual precipitation (CHIRPS), seasonal decomposition, and SAR-derived flood extent by ENSO phase (2015--2025). Precipitation in mm; flood extent in km$^2$.}
\label{tab:enso}
\centering
\footnotesize
\begin{tabular}{lcccccccc}
\toprule
\textbf{ENSO phase} & \textbf{$n$} & \textbf{Mean precip.} & \textbf{SD} & \textbf{Dry} & \textbf{Trans.} & \textbf{Veranillo} & \textbf{Wet} & \textbf{Flood extent} \\
\midrule
El Ni\~no  & 4 & 793 & 279 & 34 & 195 & 94 & 457 & 158 \\
Neutral    & 4 & 702 & 101 & 57 & 204 & 91 & 360 & 138 \\
La Ni\~na  & 3 & 904 & 285 & 39 & 221 & 136 & 506 & 189 \\
\midrule
\textbf{All years} & 11 & 790 & 210 & 44 & 206 & 104 & 435 & 159 \\
\bottomrule
\multicolumn{9}{l}{\scriptsize Seasons: Dry (Dec--Mar), Transition (Apr--Jun), Veranillo (Jul--Aug), Wet (Sep--Nov).}\\
\multicolumn{9}{l}{\scriptsize El Ni\~no: 2015 (\SI{406}{mm}), 2016, 2023, 2024 (\SI{1067}{mm}). La Ni\~na: 2020, 2021, 2022 (\SI{1196}{mm}).}\\
\multicolumn{9}{l}{\scriptsize Flood extent = cumulative annual SAR-detected water area (sum of 12 monthly composites).}
\end{tabular}
\end{table}

\textbf{Standardized Precipitation Index.} SPI-3 analysis reveals the extreme hydroclimatic oscillations that define La Guajira (Figure~\ref{fig:spi}). The year 2015 produced the five most negative SPI-3 values in the record (May--October 2015, SPI-3 $\leq -2.25$, ``extremely dry''), while 2022 generated three of the five most positive values (June--October 2022, SPI-3 $\geq 1.56$, ``severely wet''). October 2022 reached SPI-3 $= 1.95$ with a monthly total of \SI{352}{\milli\metre}---$1.8\times$ the long-term October mean (\SI{198}{\milli\metre}). Seven months exceeded the 95th percentile threshold (\SI{203}{\milli\metre}), all concentrated in October--November (the wet season peak), with the most extreme occurring during La Ni\~na years (2020, 2022, 2024). This oscillation between SPI categories confirms the ENSO-driven bipolarity of La Guajira's hydrology.

\textbf{Extreme precipitation events.} All seven 95th-percentile exceedances occurred in October (6 events) or November (1 event), spanning six of the eleven study years (Table~\ref{tab:extremes}). The single most extreme event---October 2022 (\SI{352}{\milli\metre}, exceedance ratio 1.74)---coincided with the triple-dip La Ni\~na peak. The 2024 season contributed two events (October \SI{271}{\milli\metre} and November \SI{240}{\milli\metre}), consistent with Hurricane Rafael's compound flooding. No extreme events were detected outside the September--November wet season window, highlighting the sharply concentrated flood hazard window.

\begin{table}[htbp]
\caption{Extreme precipitation events exceeding the 95th percentile ($>\SI{203}{\milli\metre/\text{month}}$) during 2015--2025. All events fall within the wet season (September--November).}
\label{tab:extremes}
\centering
\footnotesize
\begin{tabular}{cccc}
\toprule
\textbf{Year} & \textbf{Month} & \textbf{Precip.\ (mm)} & \textbf{Exceedance ratio} \\
\midrule
2016 & Oct & 226 & 1.11 \\
2018 & Oct & 231 & 1.14 \\
2020 & Nov & 223 & 1.10 \\
...
\textbf{Precipitation--flood coupling.} Monthly precipitation and SAR-detected flood extent are moderately correlated (Pearson $r = 0.51$, $p < 0.001$; Spearman $\rho = 0.45$, $p < 0.001$; $n = 132$ monthly composites), with a significant lag-1 correlation ($r = 0.36$, $p < 0.001$) indicating flood persistence approximately one month after peak rainfall. The lag-2 correlation is non-significant ($r = 0.06$, $p = 0.52$), confirming rapid flood recession consistent with semiarid terrain. The moderate (rather than strong) same-month correlation reflects two factors: (1) the spatial averaging of precipitation over the entire department obscures localized extreme rainfall that drives flooding, and (2) antecedent soil moisture modulates the rainfall--runoff relationship, with drier soils producing flashier responses for the same rainfall input.

\textbf{Drought--flood paradox.} A distinctive contribution of this study is the analysis of antecedent soil moisture conditions. During El Ni\~no droughts, soils in La Guajira become desiccated and surface-crusted, dramatically reducing infiltration capacity \citep{Alabbad2023}. The SPI record reveals that the transition from extreme drought (2015 SPI-3 $= -2.38$) to moderate/wet conditions is abrupt rather than gradual, suggesting that the rainfall increase is superimposed on soils with severely reduced permeability. This transition mechanism operates through three pathways: (i) hydrophobic surface crusting that repels initial rainfall; (ii) vegetation loss during drought that reduces canopy interception and root-zone infiltration; and (iii) sediment accumulation in ephemeral arroyos that reduces channel capacity. The 2022 and 2024 flood events---which produced catastrophic impacts despite La Guajira receiving less total rainfall than humid departments---are consistent with this drought-preconditioned amplification \citep{DroughtFloodMonitoring2024}.
...
This study demonstrates the replicability of the framework developed by \citet{EspinalMaya2026} in a contrasting environment. The CHIRPS precipitation analysis enables a rigorous quantitative comparison: La Guajira's departmental mean of \SI{790}{\milli\metre/yr} (2015--2025) is less than one-third of Antioquia's ($>\SI{2500}{\milli\metre/yr}$), yet La Guajira's interannual coefficient of variation (27\%) is more than double Antioquia's ($\sim$12\%), reflecting the dominant role of ENSO phase in controlling water availability. Key differences emerge in four domains:
...
\begin{enumerate}
    ...
    \item \textbf{Temporal dynamics:} Antioquia's bimodal seasonality (two distinct flood peaks per year) contrasts with La Guajira's unimodal pattern (single September--November peak). SAR data confirm that mean monthly flood extent peaks at \SI{27}{\kilo\metre\squared} in November and drops to \SI{4}{\kilo\metre\squared} in February---a seven-fold seasonal amplitude, compared to Antioquia's less extreme two-to-three-fold variation between peaks and troughs.
...
La Guajira exemplifies a phenomenon well-documented in arid hydrology but rarely quantified with satellite data at municipal scales: the same region that experiences extreme drought also faces catastrophic flooding, and the drought itself exacerbates subsequent flood severity. Our combined precipitation--flood analysis quantifies this paradox: La Ni\~na years produced 37\% more cumulative flood extent (\SI{189}{\kilo\metre\squared/yr}) than Neutral years (\SI{138}{\kilo\metre\squared/yr}), despite only a 29\% increase in precipitation (Table~\ref{tab:enso}). This disproportionate flood amplification is consistent with drought-preconditioned soils producing flashier runoff responses. The year 2015 produced SPI-3 values below $-2.25$ for six consecutive months (May--October), indicating the most severe meteorological drought in the study period. Just five years later, the 2020--2022 La Ni\~na reversed conditions entirely, with October 2022 reaching SPI-3 $= 1.95$ and a monthly rainfall of \SI{352}{\milli\metre}. Notably, 2024 produced the largest annual flood extent (\SI{229}{\kilo\metre\squared}) despite not being the wettest year (\SI{1067}{mm} vs. 2022's \SI{1196}{mm}), suggesting that the preceding 2023 El Ni\~no reduced soil infiltration capacity. The amplitude of this oscillation---from extremely dry to severely wet within a single ENSO cycle---is larger than documented in any comparable arid region \citep{DroughtFloodMonitoring2024}. The mechanism operates through three pathways: (1) \textit{soil crusting}---prolonged drying creates hydrophobic surface layers that repel initial rainfall, increasing surface runoff \citep{Alabbad2023}; (2) \textit{vegetation loss}---drought-induced plant mortality reduces canopy interception and root-zone infiltration; and (3) \textit{channel adjustment}---ephemeral arroyos accumulate sediment during dry periods, reducing channel capacity when flows return.

The declining LST trend ($-0.34$~\SI{}{\celsius/yr}, $p = 0.005$) observed across 2015--2025 provides indirect evidence of this hydroclimatic shift: the La Ni\~na-dominated latter half of the study period brought increased cloudiness and precipitation, reducing surface heating. This trend is not indicative of long-term climate cooling but rather reflects the sampling of ENSO phases within the 11-year window.

This paradox has direct policy implications. Traditional flood risk frameworks treat drought and flood as opposing hazards, but in La Guajira they are causally linked: investing in drought resilience (soil conservation, vegetation restoration, water harvesting) simultaneously reduces flood vulnerability by maintaining infiltration capacity and channel function. The conceptual framework of \citet{DroughtFloodMonitoring2024} for simultaneous drought--flood monitoring in Syria provides a methodological precedent, though our ENSO-stratified approach with SPI-based antecedent moisture analysis extends it to the coupled ocean-atmosphere forcing that dominates La Guajira's climate. The precipitation data reinforce this point: ENSO-phase transitions produce abrupt hydroclimatic shifts (e.g., 2015: \SI{406}{\milli\metre} $\rightarrow$ 2016: \SI{881}{\milli\metre}---a 117\% increase in a single year), with no statistically significant long-term trend in any season (Mann--Kendall $p > 0.08$), meaning that ENSO forecasting rather than climate trend extrapolation should guide risk planning.

\subsection{Wayuu Vulnerability and Environmental Justice}

The exposure analysis reveals a structural injustice: the Wayuu community, which has been identified as one of the most climate-vulnerable populations in Colombia \citep{Contreras2020, FernandezLopera2024, Contreras2019wayuu}, occupies territories in Alta and Media Guajira that are exposed to both drought and flood extremes. A systematic review of 95 studies by \citet{CervantesBenavides2025} found that indigenous communities' perceptions of climate-related risks are shaped by cultural, spiritual, and livelihood dimensions that standard hazard assessments overlook, while \citet{RamirezWayuu2024} documented how Wayuu women in La Guajira resist green colonialism from wind energy projects---demonstrating that climate adaptation interventions in Wayuu territory require culturally appropriate design. During the 2012--2016 drought, child mortality among the Wayuu reached 23.4 per 1,000---four times the national rate---driven by water scarcity, malnutrition, and limited healthcare access \citep{Contreras2020}. When La Ni\~na returns, these same communities face flash flooding along arroyos and coastal storm surge, often without access to early warning systems or evacuation infrastructure.

\citet{FernandezLopera2024} found that La Guajira has the highest climate vulnerability index of any Colombian department, and proposed culturally appropriate risk transfer mechanisms. Broader regional adaptation challenges have been comprehensively reviewed by \citet{Cavazos2024}, who identify the need to integrate indigenous knowledge with scientific assessments as a priority for Latin America and the Caribbean. Our municipality-scale flood susceptibility maps provide the missing physical hazard layer for such interventions: by identifying which Wayuu-inhabited territories face the highest compound exposure (drought $+$ flood), resources can be targeted to communities where resilience investments yield the greatest reduction in multi-hazard risk. The water governance framework proposed by Wayuu communities themselves \citep{WayuuWaterGovernance2025}---integrating traditional ecological knowledge with modern infrastructure---aligns with this approach.
...
\textbf{Scientific contribution:} We provide the first municipality-scale flood susceptibility map for a Colombian arid department, quantifying the drought--flood paradox through ENSO-stratified analysis. Processing 727 Sentinel-1 scenes into 132 monthly composites reveals that SAR-detected flood extent averaged \SI{159}{\kilo\metre\squared/yr}, with La Ni\~na years producing 37\% more flooding (\SI{189}{\kilo\metre\squared}) than Neutral years (\SI{138}{\kilo\metre\squared})---a disproportionate amplification relative to the 29\% precipitation increase. CHIRPS analysis (2015--2025) confirms extreme ENSO sensitivity: La Ni\~na years averaged \SI{904}{mm/yr} versus \SI{793}{mm/yr} for El Ni\~no, with the 2022 triple-dip La Ni\~na producing \SI{1196}{mm}---nearly three times the 2015 El Ni\~no minimum (\SI{406}{mm}). SPI-3 analysis documents oscillations from ``extremely dry'' (SPI-3 $= -2.38$, 2015) to ``severely wet'' (SPI-3 $= 1.95$, 2022), confirming that antecedent drought conditions amplify subsequent flood severity---a mechanism with direct implications for multi-hazard early warning systems.
...
The following supplementary materials are available online: Table~S1 (Flood Risk Index for all 15 municipalities), Table~S2 (Annual precipitation by season, 2015--2025), Table~S3 (SPI-3 monthly time series), Figures~S1--S3 (SHAP dependence plots for HAND, flood frequency, and elevation), Figure~S4 (per-fold ROC curves), Figure~S5 (Sand Exclusion Layer validation against manual digitisation), Figure~S6 (MODIS LST annual trend with Mann-Kendall significance), and the complete Google Earth Engine processing scripts. All code is available at \url{https://github.com/Cespial/guajira-flood-risk}.
...
SPI$_{\text{topo}}$ & Stream Power Index (topographic)\\
SPI$_{\text{precip}}$ & Standardized Precipitation Index\\
